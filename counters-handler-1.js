 ...
 * ✅ [إصلاح جذري] فصل مسار العدادات: inventory → counters
 *    (DB.inventory في index.html مصفوفة ورديات — مسار مختلف تماماً)
 *
 ...
 *              لمنع تعارضه مع rtdb.ref('.info/connected') في index.html
...
// ✅ [v2.4 محدَّث] يعتمد مباشرة على navigator.onLine ومرجع Firebase rtdb
//    الرئيسي المُعرَّف في firebase-config.js — بدون متغيرات محلية معزولة
let _firebaseConnected = null; // null = لم يُحدَّد بعد من Firebase

// تحديث _firebaseConnected فور توفر Firebase rtdb
// نسجّل المستمع هنا مرة واحدة لأن index.html لا يكشف _isOnline خارجياً بشكل موثوق
if (typeof rtdb !== 'undefined' && rtdb) {
  rtdb.ref('.info/connected').on('value', snap => {
    _firebaseConnected = snap.val() === true;
    window._isOnline = _firebaseConnected; // مشاركة مع index.html
  });
}
 ...
 *
 * منطق الأولوية:
 *   1. Firebase rtdb.info/connected → الأدق (يكشف Lie-Fi وانقطاع WebSocket)
 *   2. window._isOnline (مُحدَّث من Firebase في index.html) → احتياطي
 *   3. navigator.onLine → احتياطي أخير دائم التوفر
 *
 * ✅ لا يعتمد على متغير محلي معزول قد يُجمَّد
 * ✅ لا يُنشئ تعارضاً مع منطق الشبكة في index.html
 */
const checkOnlineStatus = () => {
  if (typeof _firebaseConnected === 'boolean') return _firebaseConnected;
...
//         تعارض مع rtdb.ref('.info/connected') في index.html
//         وأعاد تحميل Firebase أثناء معالجته الداخلية للأوفلاين
// ✅ الآن: index.html هو المسؤول الوحيد عن مستمعَي الشبكة
 ...
 *   ✅ تستدعي _updateSyncIndicator() من index.html للرأس (إن كانت متاحة)
...
function updateSyncIndicator() {
  // ── [إصلاح] حماية DOM: لا نفعل شيئاً إن لم يكتمل بناء الصفحة بعد ──
  // يمنع Uncaught TypeError عند تنفيذ السكريبت مبكراً
  ...
  // يستخدم _updateSyncIndicator() من index.html إن كانت متاحة
  ...
  if (typeof _updateSyncIndicator === 'function') {
    ...
    if (headerInd) {   // ✅ [إصلاح] حماية: التأكد من وجود العنصر أولاً
  ...
  if (cardInd) {   // ✅ [إصلاح] حماية: التأكد من وجود العنصر أولاً
...
async function loadCountersFromFirebase() {
  ...
  try {
    ...
    const snapshot = await new Promise((resolve, reject) => {
      DB_REF.child('counters').once('value', resolve, reject);
    });
...
function monitorCountersChanges() {
  ...
  DB_REF.child('counters').on('value', snapshot => {
...
async function updateCounter(pumpType, liters) {
  ...
  DB_REF.child('counters').child(pumpType).transaction(
    ...
    (error, committed) => {
      if (error || !committed) {
        ...
        // ✅ [إصلاح] أُزيل: DB_REF.child('counters').child('timestamp').set(Date.now())
        // كان يخرق Firebase Security Rules التي تسمح فقط بـ (diesel|fuel91|fuel95)
        // كتابة 'timestamp' كانت تُعيد تشغيل الـ onValue listener وتسبب حلقة لا نهائية
 ...
 *      تقرأه _flushPendingSync() في index.html عند المزامنة الشاملة
...
async function resetCountersForNewDay() {
  ...
  try {
    ...
    if (roleSnap.val() !== 'owner') {
      console.error('❌ ليس لديك صلاحيات كافية — هذه الصلاحية للمالك فقط');
      return false;
    }
    ...
    // ✅ [إصلاح] أُزيل timestamp — يخرق Rules التي تسمح فقط بـ (diesel|fuel91|fuel95)
    await new Promise((resolve, reject) => {
      DB_REF.child('counters').set({
        diesel: 0,
        fuel91: 0,
        fuel95: 0
      }, err => err ? reject(err) : resolve());
...
// ══════════════════════════════════════════════════════════════════
// [FIX] تفريغ طابور الأوفلاين — كانت queueCounterUpdate() تحفظ العمليات
// الفاشلة في counters_pending_ops لكن لا شيء كان يقرأها لاحقاً؛ عند عودة
// الاتصال كانت تُفقد نهائياً من Firebase (تبقى ظاهرة محلياً فقط حتى
// تحديث الصفحة). هذه الدالة تُعيد إرسال كل عملية معلّقة فعلياً، وتحذفها
// من IndexedDB فقط بعد نجاح الكتابة في Firebase.
// ══════════════════════════════════════════════════════════════════
function drainPendingCounterOps() {
  if (!window.indexedDB || !checkOnlineStatus() || !DB_REF) return;

  const request = indexedDB.open('FuelStationDB', 1);
  request.onerror = (e) => console.warn('⚠️ تعذر فتح IndexedDB لتفريغ الطابور:', e.target?.error);

  request.onsuccess = (e) => {
    const db = e.target.result;
    if (!db.objectStoreNames.contains('counters_pending_ops')) return;

    const tx = db.transaction('counters_pending_ops', 'readonly');
    const getAllReq = tx.objectStore('counters_pending_ops').getAll();

    getAllReq.onsuccess = () => {
      const pending = getAllReq.result || [];
      if (pending.length === 0) return;
      console.log(`📤 إعادة إرسال ${pending.length} عملية معلّقة من الأوفلاين...`);

      pending.forEach(item => {
        const { pumpType, liters } = item.data || {};
        if (!pumpType || typeof liters !== 'number') return;

        DB_REF.child('counters').child(pumpType).transaction(
          currentValue => (currentValue || 0) + liters,
          (error, committed) => {
            if (!error && committed) {
              // نجحت المزامنة — احذف العنصر من الطابور فقط الآن
              const delTx = db.transaction('counters_pending_ops', 'readwrite');
              delTx.objectStore('counters_pending_ops').delete(item.syncId);
              delTx.oncomplete = () => console.log('✅ تمت مزامنة عملية معلّقة وحذفها من الطابور:', item.syncId);
            } else {
              console.warn('⚠️ فشلت إعادة إرسال عملية معلّقة، ستُحاوَل لاحقاً:', item.syncId, error);
            }
          }
        );
      });
    };
    getAllReq.onerror = (err) => console.warn('⚠️ خطأ في قراءة الطابور المعلّق:', err);
  };
}
...
document.addEventListener('userLoggedIn', () => {
  ...
  drainPendingCounterOps(); // [FIX] إعادة إرسال أي تحديثات علقت أثناء انقطاع الاتصال
});

// [FIX] أيضاً عند عودة الاتصال فعلياً أثناء الجلسة (لا الدخول فقط) —
// checkOnlineStatus يعتمد على rtdb.ref('.info/connected') في index.html
document.addEventListener('connectionRestored', () => drainPendingCounterOps());
...
window.CountersAPI = {
  ...
  updateCounter,
  queueCounterUpdate,
  drainPendingCounterOps,     // [FIX] تفريغ طابور الأوفلاين المعلّق
  resetCountersForNewDay,
  updateSyncIndicator,        // [جديد v2.4] مكشوفة لاستدعائها من index.html
